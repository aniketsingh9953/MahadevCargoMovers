...
async function main() {
  ...
  // 2. Create Consignment Notes Table
  await db.exec(`
  ...
  // 3. Create the default admin user if none exists yet.
  ...
  // Give connection a tiny window to completely finish statements before closing
  setTimeout(async () => {
    try {
      await db.close();
    } catch (e) {}
