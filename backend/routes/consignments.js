...
router.get('/', asyncHandler(async (req, res) => {
  ...
  if (q) {
    ...
    rows = await db.prepare('SELECT * FROM consignment_notes ORDER BY id DESC').all();
  }
  res.json(rows);
}));

// GET /api/consignments/next-lr-no — suggests the next LR number
router.get('/next-lr-no', asyncHandler(async (req, res) => {
...
router.get('/:id', asyncHandler(async (req, res) => {
  const row = await db.prepare('SELECT * FROM consignment_notes WHERE id = ?').get(req.params.id);
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const existing = await db.prepare('SELECT id FROM consignment_notes WHERE lr_no = ?').get(body.lr_no);
  if (existing) {
  ...
  try {
    ...
    const created = await db.prepare('SELECT * FROM consignment_notes WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', asyncHandler(async (req, res) => {
  const body = req.body;
  const existing = await db.prepare('SELECT * FROM consignment_notes WHERE id = ?').get(req.params.id);
...
router.delete('/:id', asyncHandler(async (req, res) => {
  const existing = await db.prepare('SELECT id FROM consignment_notes WHERE id = ?').get(req.params.id);
  if (!existing) return res.status(404).json({ error: 'Consignment note not found.' });

  await db.prepare('DELETE FROM consignment_notes WHERE id = ?').run(req.params.id);
